...
class ResNet269(nn.Module):
    def __init__(self, num_classes=20):
        ...
        # ResNet的4个主层，block数量
        self.layer1 = self._make_layer(Bottleneck, 64, 6)   # 
        self.layer2 = self._make_layer(Bottleneck, 128, 12, stride=2)  # 
        self.layer3 = self._make_layer(Bottleneck, 256, 64, stride=2)  # 
        self.layer4 = self._make_layer(Bottleneck, 512, 6, stride=2)   #
...
class _PSPModule(nn.Module):
    def __init__(self, in_channels, out_channels, pool_size):
        ...
        self.SSM = nn.Sequential(
            Mamba(in_channels, d_state=16, expand=2),
            nn.PReLU())
        ###################
